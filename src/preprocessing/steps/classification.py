...
# Some bundles ship the SavedModel weights as ``variables-001.data-*`` while
# TensorFlow expects ``variables.data-*``.
...
def _ensure_savedmodel_variable_filenames(model_dir: Path) -> None:
    ...
    ``variables.data-...`` (per ``variables.index``). Copy missing canonical names so
    the model loads without manual filesystem fixes.
    """
...
def classify_syllables(
    ...
    For each syllable: loads the recording audio (with caching across rows of the
    same recording), extracts the segment, applies a 30 kHz high-pass filter,
    computes the STFT, resizes to 128x128, and feeds it to the model. Results
    are grouped per recording as :class:`legacy.statistics_generator.sample`
    objects.
    """
...
def write_syllable_numbers(file_path: str, syl_num: List[int]) -> None:
    ...
    Uses pandas + a single ``to_excel`` pass. Per-cell openpyxl loops are far
    too slow for tens of thousands of syllable rows (would stall the run for
    hours on large workbooks).
    """
...
def run_classification(
    ...
    *,
    ...
    Args:
        ...
        model_path: Path to the Keras model weights file (.h6) or SavedModel dir.
        age_syl, matgen_syl, pupgen_syl, mother_syl, name_syl, sex_syl,
        session_syl, rec_num_syl, start_syl, end_syl: per-syllable metadata lists.
        wav_path_syl: Optional ``Path`` column from the segmentation workbook;
            when provided, the resolver prefers these (already-validated paths).
        logger: Optional logger instance.
        year_audio_root: Optional client-layout year directory for relative
            ``USV_Recordings/<year>/...`` paths in ``wav_path_syl``.
        progress_hook: Optional callable receiving (done, total, phase[, chunk_end]).
        save_npy: When True, dump ``samples`` to a sibling ``.npy`` file
            (kept ``True`` by default so tabular training that depends on
            the ``.npy`` artifact continues to work).
        stage_callback: Optional callback receiving stage names
            (``load_model`` → ``model_ready`` → ``postprocess`` → ``write_excel``).
