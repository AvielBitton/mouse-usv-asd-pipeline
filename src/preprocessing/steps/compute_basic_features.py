...
def compute_basic_features(
    ...
    Two execution modes:
      - In-memory (default): pass ``signal_vec`` containing pre-loaded waveforms.
      - Streaming (memory-efficient): pass ``audio_paths`` with one path per
        recording; each WAV is loaded once, used, and freed before the next.
    ...
    Args:
        ...
        audio_paths: Optional list of WAV paths (one per recording) for the
            memory-efficient mode. When set, ``signal_vec`` is ignored.
    ...
    # Canonical data row count from the segmentation table (exclude header).
    # Guards against accidental row inflation from mismatched feature vectors.
