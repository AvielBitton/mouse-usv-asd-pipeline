...
def _welch_psd(seg, rate, nperseg=1024, noverlap=625):
    ...
    (e.g. end of file, or st1/st2 near the end of the recording), scipy uses
    a smaller effective nperseg; a fixed *noverlap* would then become invalid.
    """
...
def StartEndFreq_from_paths(
    ...
    """
    Same as :func:`StartEndFreq` but loads each recording from disk one at a
    time. Lower peak RAM than holding all of ``SignalVec`` in memory, which
    matters for multi-thousand-row workbooks.
    """
    _log = logger or logging.getLogger(__name__)
    _fail_n = [0]
    startF = []
    ...
    # Preserves output order while avoiding O(siz * n_syllables) scans.
    syllable_idx_by_key = {}
    use_index = True
    try:
        for i in range(len(motherSyl)):
