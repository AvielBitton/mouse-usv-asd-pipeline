...
def _resolve_existing_wav(year, mother, matgen, name, pupgen, age, session, rec_num) -> Optional[Path]:
  """Resolve a WAV path using the same builder + extension search as the loader step."""
  base = build_recording_base_path(
      'USV_Recordings', year, mother, matgen, name, pupgen,
      age, session, rec_num,
  )
  return resolve_wav_path(base)


def _resolve_from_path_cell(path_cell, year, year_audio_root: Optional[Path]) -> Optional[Path]:
  """Try the workbook ``Path`` cell as-is; fall back to ``year_audio_root`` for relative paths."""
...
def Syl_Class_Vec(
  ...
  """Generate per-syllable spectrograms and run the CNN classifier in batched chunks.

  Behaviour highlights:
    - Audio is loaded once per recording (``loaded_path`` cache), avoiding the
      O(syllables) ``librosa.load`` calls that plagued earlier versions.
    - Spectrograms for every syllable are stacked and predicted in chunks of
      :data:`_GLOBAL_INFERENCE_CHUNK` (capped lower for huge jobs so progress
      reports update frequently); each ``model.predict`` runs with
      :data:`_PREDICT_BATCH_SIZE`.
    - When ``wav_paths`` is provided (e.g. the ``Path`` column from the
      segmentation workbook), the resolver prefers it; otherwise it falls
      back to :func:`build_recording_base_path` + :func:`resolve_wav_path`,
      which matches the loader step's resolution semantics.
  """
  ...
  for j in range(n_mother):
    resolved = _resolve_from_path_cell(wav_paths[j], year, year_audio_root) if (
        wav_paths is not None and j < len(wav_paths)
    ) else None
    if resolved is None:
      resolved = _resolve_existing_wav(
  ...
  for i in range(n_mother):
    resolved = _resolve_from_path_cell(wav_paths[i], year, year_audio_root) if (
        wav_paths is not None and i < len(wav_paths)
    ) else None
    if resolved is None:
      resolved = _resolve_existing_wav(
    ...
    if time_diff[i] < max_time:
      ...
      trimmed = rec[round((start[i])*rate):round((finish[i])*rate)]  # trim syllable
      syl = np.append(silence, trimmed)  # normalize length to max syllable
    ...
    # STFT creation
    D = np.abs(librosa.stft(syl, n_fft=2048, hop_length=128, win_length=512, window='hamming'))  # absolute value spectrogram
    D = cv2.resize(D, dsize=(128, 128), interpolation=cv2.INTER_CUBIC)  # 128x128 for the CNN; bicubic 4x4 neighbourhood
    D = D - 0.02*np.mean(D)
  ...
  # Stack all syllable tensors, then ``model.predict`` in chunks. Large chunks keep
  # the GPU busy; for small jobs (few syllables) use a small chunk so progress_hook
  # can show 1/N … N/N like the spectrogram phase, instead of jumping straight to
  # N/N in a single step.
